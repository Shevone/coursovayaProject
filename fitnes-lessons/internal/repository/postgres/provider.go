...
func (s Storage) GetAllLessonsFromDb(ctx context.Context, page int32, limit int32) ([]models.Lesson, error) {
	...
	queryRow := "SELECT id, title,date_and_time,trainer_id,available_seats,description,difficult FROM lessons ORDER BY date_and_time offset $1 limit $2"
...
func (s Storage) GetLessonsByTrainerIdFromDb(ctx context.Context, trainerId int64, page int32, limit int32) ([]models.Lesson, error) {
	const op = "SQLite.GetAllLessons"
	queryRow := "SELECT id, title,date_and_time,trainer_id,available_seats,description,difficult FROM lessons WHERE strftime('%m-%d %H:%M', date_and_time) >= strftime('%m-%d %H:%M', 'now') AND trainer_id = $1 ORDER BY date_and_time LIMIT $2, $3"
...
func (s Storage) GetLessonsByUserIdFromDb(ctx context.Context, userId int64, page int32, limit int32) ([]models.Lesson, error) {
	const op = "SQLite.GetAllLessons"
	queryRowGetUserLessons := "SELECT lesson_id FROM student_lessons where user_id = $1"
	...
	for _, lessonId := range lessonIdList {
		queryRow := "SELECT id, title,date_and_time,trainer_id,available_seats,description,difficult FROM lessons WHERE id = $1 ORDER BY date_and_time DESC  offset $2 LIMIT $3"
...
func (s Storage) GetLessonFromDb(ctx context.Context, lessonId int64) (*models.Lesson, error) {
	...
	queryRow := "SELECT id, title,date_and_time,trainer_id,available_seats,description,difficult FROM lessons WHERE id = $1"
	lesson := &models.Lesson{}
	var timeAndDate string
