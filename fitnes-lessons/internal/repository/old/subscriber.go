...
//	stmt, err := s.db.Prepare("UPDATE lessons SET is_complete = false WHERE id = $1")
//	if err != nil {
...
//	queryRow := "SELECT lessons.is_complete, lessons.available_seats from lessons WHERE id = $1"
//	var isComplete bool
...
//	queryRow = "SELECT count(*) from student_lessons where user_id = $1 AND lesson_id = $2"
//	var count int
...
//		return "", fmt.Errorf("%s: %w", op, err)
//	}
//	stmt, err = s.db.Prepare("UPDATE lessons SET available_seats = $1 WHERE lessons.id = $2")
//	if err != nil {
//		return "", fmt.Errorf("%s: %w", op, err)
//	}
